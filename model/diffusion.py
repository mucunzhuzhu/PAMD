...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t,
                                 noise):
        """
    ...
    def predict_noise_from_start(self, x_t, t,
                                 x0):
        return (
    ...
    def model_predictions(self, x, cond, normalizer, t, weight=None,
                          clip_x_start=False):
        weight = weight if weight is not None else self.guidance_weight

        model_output = self.model.guided_forward(x, cond, normalizer,t,
                                                 weight)
    ...
    def p_mean_variance(self, x, cond, normalizer,t):
        ...
        x_recon = self.predict_start_from_noise(
            x, t=t, noise=self.model.guided_forward(x, cond, normalizer,t, weight)
        )
    ...
    @torch.no_grad()
    def p_sample(self, x, cond,normalizer, t):
        b, *_, device = *x.shape, x.device

        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, cond=cond,normalizer=normalizer, t=t
        )
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, cond, normalizer,**kwargs):
        batch, device, total_timesteps, sampling_timesteps, eta = shape[
                                                                      0], self.betas.device, self.n_timestep, 50, 1
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(x, cond,normalizer, time_cond, clip_x_start=self.clip_denoised)
    ...
    @torch.no_grad()
    def long_ddim_sample(self, shape, cond, normalizer,
                         **kwargs):
        batch, device, total_timesteps, sampling_timesteps, eta = shape[
                                                                      0], self.betas.device, self.n_timestep, 50, 1
        if batch == 1:
            return self.ddim_sample(shape, cond,normalizer)


        times = torch.linspace(-1, total_timesteps - 1,
                               steps=sampling_timesteps + 1)  # {list:51}   # [-1, 0, 1, 2, ..., T-1] when sampling_timesteps == total_timesteps
        times = list(reversed(times.int().tolist()))  # [999,979,959,...19,-1]
        weights = np.clip(np.linspace(0, self.guidance_weight * 2, sampling_timesteps), None,
        ...
        for time, time_next, weight in tqdm(time_pairs, desc='sampling loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            pred_noise, x_start, *_ = self.model_predictions(x, cond, normalizer, time_cond, weight=weight,
    ...
    def inpaint_loop(
        ...
        for i in tqdm(reversed(range(0, start_point))):
            ...
            x, _ = self.p_sample(x, cond, normalizer,timesteps)
    ...
    def p_losses(self, x_start, cond, normalizer, t):
        noise = torch.randn_like(x_start)

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)

        # reconstruct
        x_recon = self.model(x_noisy, cond, normalizer, t,cond_drop_prob=self.cond_drop_prob)
        ...
        # X, Q
        model_x = model_out[:, :, :3]
        model_q = ax_from_6v(model_out[:, :, 3:].reshape(b, s, -1, 6)) # (b,s,24,3)
        target_x = target[:, :, :3]
        ...
        pose_NDF = model_q.reshape(b,s,24,3)[:,:,1:,:] # (b,s,23,3)
        pose_quat = axis_angle_to_quaternion(pose_NDF.reshape(-1,23,3)[:,:21])
        dis_val = self.pose_prior(pose_quat,train=False)['dist_pred']
        loss_NDF = torch.mean(dis_val)
    ...
    def loss(self, x, cond, normalizer,t_override=None):
        batch_size = len(x)
        ...
        return self.p_losses(x, cond, normalizer, t)

    def forward(self, x, cond, normalizer, t_override=None):
        return self.loss(x, cond, normalizer, t_override)
